...
from octodiff.train.checkpointing import load_checkpoint, save_checkpoint
from octodiff.train.configuration import OctodiffTrainingConfig, load_training_config
from octodiff.train.data import build_dataloaders
from octodiff.train.logging_utils import log_event, setup_logging
from octodiff.train.utils import prepare_tokenizer, set_seed
...
def build_model(
    ...
    model_config = OctodiffConfig(**model_kwargs)
    model = OctodiffForDiffusionLM(model_config)
...
def train(model_config: OctodiffConfig, train_cfg: OctodiffTrainingConfig, logger: Optional[logging.Logger] = None) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train Octodiff diffusion language model")
    parser.add_argument(
        "--config",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional path to an OctodiffConfig JSON file overriding model hyperparameters.",
    )
...
if __name__ == "__main__":
    ...
    model_config = OctodiffConfig.from_json_file(args.model_config) if args.model_config else OctodiffConfig()
